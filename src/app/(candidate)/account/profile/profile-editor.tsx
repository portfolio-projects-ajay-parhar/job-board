...
export function ProfileEditor() {
  ...
  if (!profile) return <p className="text-sm text-slate-500 dark:text-slate-400">Loading…</p>;

  const set = (patch: Partial<Profile>) => setProfile((p) => ({ ...(p as Profile), ...patch }));

  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <label className={label}>
          <span className="font-medium text-slate-700 dark:text-slate-300">Headline</span>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <label className={label}>
          <span className="font-medium text-slate-700 dark:text-slate-300">Years of experience</span>
          <input
            type="number"
            min={0}
            max={60}
      ...
      <label className={label}>
        <span className="font-medium text-slate-700 dark:text-slate-300">Skills (comma-separated, max 20)</span>
        <input value={skillsText} onChange={(e) => setSkillsText(e.target.value)} className={inputClass} placeholder="React, TypeScript, Node.js" />
      </label>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <label className={label}>
          <span className="font-medium text-slate-700 dark:text-slate-300">Portfolio URL</span>
