    ...
    <div>
      <div className="flex gap-4 overflow-x-auto pb-4">
        {COLUMNS.map((col) => {
          ...
          return (
            <div key={col} className={`min-w-64 flex-1 rounded-xl border-t-4 bg-slate-100 p-3 ${COLUMN_STYLES[col]}`}>
              ...
              <div className="flex flex-col gap-2">
                {apps.map((a) => {
                  ...
                  return (
                    <div key={a.id} className="rounded-lg border border-slate-200 bg-white p-3 text-sm shadow-sm">
                      <button onClick={() => setDetail(open ? null : a.id)} className="w-full text-left">
                        <p className="font-medium text-slate-900">{a.candidate.name ?? a.candidate.email}</p>
                        {a.candidate.headline && <p className="mt-0.5 text-xs text-slate-500">{a.candidate.headline}</p>}
                        <div className="mt-1 flex flex-wrap gap-1">
                          {a.candidate.skills.slice(0, 3).map((s) => (
                            <span key={s} className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-600">
                              {s}
                            </span>
                          ))}
                        </div>
                      ...
                      {open && (
                        <div className="mt-3 border-t border-slate-100 pt-3">
                          ...
                          <p className="mt-2 text-xs font-semibold uppercase text-slate-400">Resume</p>
                          {a.resume ? (
                            <a
                          ...
                          {legal.length > 0 && (
                            <div className="mt-3 flex flex-col gap-2">
                              ...
                              <p className="text-[10px] text-slate-400">Candidate will be notified by email.</p>
                            </div>
                          )}
                        </div>
                      )}
                ...
                {apps.length === 0 && <p className="py-2 text-center text-xs text-slate-400">Empty</p>}
              </div>
            </div>
          );
        })}
