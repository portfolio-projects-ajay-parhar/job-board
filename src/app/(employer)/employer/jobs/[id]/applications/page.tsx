  ...
  return (
    <div>
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        <Link href={`/jobs/${job?.slug ?? ""}`} className="text-sm font-medium text-blue-600 hover:underline">
          View public posting →
        </Link>
      </div>
      <div className="mt-6">{job ? <PipelineBoard jobId={id} /> : <p className="text-sm text-slate-500">Job not found.</p>}</div>
    </div>
  );
}
