...
export function JobForm({ jobId }: { jobId?: string }) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex max-w-3xl flex-col gap-5 rounded-xl border border-slate-200 bg-white p-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <label className={label}>
          <span className="font-medium text-slate-700">Salary period</span>
      ...
      <div className="grid grid-cols-3 gap-4">
        <label className={label}>
          <span className="font-medium text-slate-700">Min salary (USD)</span>
          <input value={minDollars} onChange={(e) => setMinDollars(e.target.value)} className={inputClass} placeholder="120000" inputMode="decimal" />
        </label>
        <label className={label}>
          <span className="font-medium text-slate-700">Max salary (USD)</span>
      ...
      <label className={label}>
        <span className="font-medium text-slate-700">Description (HTML, sanitized on save)</span>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/employer/jobs" className="rounded-md border border-slate-300 px-4 py-2 text-sm hover:bg-slate-100">
            Cancel
          </Link>
